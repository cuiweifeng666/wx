...
Page({
  ...
  data: {
    head: {
      ...
      headOff: 'true',
      titleTrue:'true',
      headRightTitle:'位置间距走势图',
      headLinkUrl: '../attr/attr?type=3d'
    },
    question: {
      off: false,
      title: '位置间距',
      content: '即为（百位减十位）减（十位减个位）的绝对值。如：566，5-6=1 6-6=0 1-0=1那么566这注号码的位置间距则为1。'
    },
  ...
  onLoad: function (options) {
    ...
    wx.setStorageSync('link', 'https://m.cjcp.com.cn/zst/cjw3d/3d_weishu-wjj.html')
  ...
  sure: function () {
    ...
    // }
    if (arr.length > 0) {
      tiao = 'weizhijianju_0_' + arr.join(',');
    }
    for (var s = 0; s < a.length; s++) {
      if (a[s] == undefined || a[s] == 'none') {
      } else {
        if (tiao.length == 0) {
          tiao += 'weizhijianju_' + (duty[s]) + '_' + a[s];
        } else {
          tiao += '#weizhijianju_' + (duty[s]) + '_' + a[s];
        }
      }
